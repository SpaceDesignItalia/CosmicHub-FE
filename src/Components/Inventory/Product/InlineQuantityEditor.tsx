...
export default function InlineQuantityEditor({ product, onUpdate }: InlineQuantityEditorProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editValue, setEditValue] = useState((product.quantity || 0).toString());
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log("Aggiornamento quantità per prodotto:", product.product_id, "Nuova quantità:", newQuantity);
      
      try {
        // Proviamo prima con un endpoint specifico per l'aggiornamento quantità
        await axios.put(`/Product/PUT/UpdateProductQuantity/${product.product_id}`, updateData);
      } catch (apiError: any) {
        ...
        if (apiError.response?.status === 404 || apiError.code === 'ERR_NETWORK') {
          console.log("Endpoint specifico non trovato, provo con aggiornamento generico...");
          await axios.put(`/Product/PUT/UpdateProduct/${product.product_id}`, updateData);
        } else {
          throw apiError;
        }
      }
      ...
      <Tooltip 
        content={
          <div className="px-1 py-2">
            ...
            <div className="text-tiny">Doppio click per modificare la quantità</div>
          </div>
        }
        delay={1000}
        closeDelay={0}
      >
        ...
        >
          <div className="flex items-center gap-1">
            {quantity}
            {showSuccess && (
              <Icon icon="solar:check-circle-bold" className="text-success text-sm animate-bounce" />
            )}
