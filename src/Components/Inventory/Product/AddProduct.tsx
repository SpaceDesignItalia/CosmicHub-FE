...
export default function AddProduct() {
  ...
  const generateSKU = (productName: string, category: string = "", brand: string = "") => {
    ...
    if (words.length === 1) {
      ...
      if (word.length <= 3) {
        ...
        sku = consonants.length >= 3 ? consonants.substring(0, 3) : word.substring(0, 3);
      }
    ...
    const now = new Date();
    const timeComponent = (now.getTime() % 1000000).toString().slice(-3).padStart(3, "0");
  ...
  useEffect(() => {
    if (formData.name && (!formData.sku || isSkuAutoGenerated)) {
      const generatedSku = generateSKU(formData.name, formData.category, formData.brand);
      if (generatedSku) {
                  ...
                  <label className="block text-sm font-medium mb-2">
                    ...
                    {isSkuAutoGenerated && (
                      <Chip 
                        ...
                        color="primary" 
                        className="ml-2"
                        startContent={<Icon icon="solar:magic-stick-3-bold" width={14} />}
                      >
                        Auto-generato
                      </Chip>
                    )}
                  ...
                  <div className="flex gap-2">
                    ...
                    <Tooltip content="Rigenera SKU automaticamente">
                      <Button
                        ...
                        isIconOnly
                        onClick={() => {
                          const newSku = generateSKU(formData.name, formData.category, formData.brand);
                          if (newSku) {
                    ...
                    <Tooltip 
                      content={
                        <div className="px-1 py-2 max-w-xs">
                          <div className="text-small font-bold mb-2">Come funziona la generazione SKU?</div>
                          <div className="text-tiny space-y-1">
                            ...
                            <p className="mt-2 text-warning">Il numero finale è sempre univoco</p>
                          </div>
                        </div>
                      }
                      delay={0}
                    ...
                    >
                      <Button
                        ...
                        size="sm"
                      >
                        <Icon icon="solar:info-circle-bold" className="text-default-400" />
                      </Button>
                    </Tooltip>
                  </div>
                  {formData.name && !formData.sku && (
                    <p className="text-xs text-default-400 mt-1">
                      Lo SKU verrà generato automaticamente quando inserisci il nome del prodotto
                    </p>
                  )}
