...
const WarehouseDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWarehouseDetails = async () => {
      try {
        ...
        const warehouseData = {
          ...response.data,
          // Garantiamo che name e location siano sempre valorizzati
          name:
            response.data.WarehouseName ||
            response.data.name ||
            "Nome non disponibile",
          location:
            response.data.WarehouseAdress ||
        ...
        if (creatorId) {
          await fetchEmployeeDetails(creatorId);
        }

        // Recupera sempre i dettagli dell'azienda usando il company_id del magazzino
        if (warehouseData.company_id) {
          await fetchCompanyDetails(warehouseData.company_id);
        } else {
          setLoading(false);
        }
  ...
  // Funzione di refetch per aggiornare i dati del magazzino
  const refetchWarehouseData = async () => {
    if (!UUID) return;
    ...
    try {
      ...
      console.error("Errore nel refetch del magazzino:", err);
    } finally {
  ...
  const handleDeactivateWarehouse = async () => {
    ...
    try {
      ...
      // Mostra il messaggio di successo
      setSuccessMessage("Magazzino disattivato con successo!");
      ...
      // Ricarica i dati del magazzino per mostrare lo stato aggiornato
      setTimeout(() => {
        refetchWarehouseData();
      }, 1500);
    } catch (error) {
      console.error("Errore durante la disattivazione del magazzino:", error);
      setIsDeleteLoading(false);
      onDeactivateClose();
    }
  };

  // Riattivazione del magazzino
  const handleReactivateWarehouse = async () => {
    if (!UUID) return;

    setIsDeleteLoading(true);
    try {
      await axios.put(`/Warehouse/UPDATE/ReactivateWarehouse`, {
        warehouse_uuid: UUID,
      });

      // Mostra il messaggio di successo
      setSuccessMessage("Magazzino riattivato con successo!");
      ...
      console.error("Errore durante la riattivazione del magazzino:", error);
      setIsDeleteLoading(false);
      onDeactivateClose();
    }
  };
  ...
  return (
    <>
          ...
          <div className="mb-6 flex items-center rounded-lg bg-danger-50 p-4 text-danger-700">
            ...
            <div>
              ...
              <p className="text-sm">
                ...
                riattivarlo dal pulsante azioni o pagina di modifica.
              </p>
            </div>
          </div>
        )}
                    ...
                    <div>
                      ...
                      <p className="text-foreground font-medium">
                        {createdByUser
                          ...
                          : `ID: ${
                              warehouse.CreatedBy || warehouse.created_by
                            }`}
                      </p>
                    </div>
                    <div>
                        ...
                        <div className="flex h-full w-full items-center justify-center rounded-lg bg-default-100">
                          <div className="text-center p-6">
                            ...
                            <p className="text-default-500 mt-2">
                              Non ci sono informazioni sufficienti sulla
      ...
      {/* Modal di conferma disattivazione/riattivazione */}
          ...
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <p>
                  Sei sicuro di voler{" "}
                  {warehouse?.IsActive === false ? "riattivare" : "disattivare"}{" "}
                  il magazzino <strong>{warehouse?.name}</strong>?
                </p>
                <p className="text-sm text-default-500 mt-2">
                  {warehouse?.IsActive === false
                    ? "Il magazzino tornerà ad essere utilizzabile per tutte le operazioni."
                    : "Il magazzino sarà reso non utilizzabile ma potrà essere riattivato in seguito. Tutti i dati rimarranno salvati."}
                </p>
              </ModalBody>
              <ModalFooter>
              ...
              <ModalHeader className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  <Icon
                    ...
                    width={24}
                  />
                  <span>Elimina Magazzino</span>
                </div>
              </ModalHeader>
              <ModalBody>
                <p>
                  Sei sicuro di voler eliminare definitivamente il magazzino{" "}
                  <strong>{warehouse?.name}</strong>?
                </p>
                <p className="text-sm text-danger mt-2">
                  ⚠️ Questa azione è irreversibile. Tutti i dati del magazzino
      ...
      {successMessage && (
        <div className="fixed bottom-4 right-4 z-50">
          <div className="bg-success text-success-foreground px-4 py-2 rounded-lg shadow-lg">
            {successMessage}
          </div>
        </div>
      )}

      {/* Overlay spinner per refetch */}
      {isRefetching && (
        <div
        ...
        >
          <div
            className="p-8 rounded-xl shadow-2xl flex flex-col items-center min-w-[280px]"
            style={{
