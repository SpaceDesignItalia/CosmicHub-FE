  ...
  useEffect(() => {
    ...
    if (Object.keys(params).length > 0) {
      const eventType = eventTypeOptions.find(
        (t) => t.key === params.intervention_type
      );

      setFormData((prev) => ({
        ...prev,
        customer_id: params.customer_id || prev.customer_id,
        ...
        estimated_duration:
          parseInt(params.estimated_duration) ||
          eventType?.duration ||
          prev.estimated_duration,
        notes: params.notes || prev.notes,
        location: params.location || params.customer_address || prev.location,
        assigned_technician:
          params.assigned_technician ||
          params.preferred_technician ||
          prev.assigned_technician,
        assigned_technician_id:
          params.assigned_technician_id ||
          params.preferred_technician_id ||
          prev.assigned_technician_id,
        contact_method:
          (params.preferred_contact_method as any) || prev.contact_method,
        customer_type: (params.customer_type as any) || prev.customer_type,
        from_external:
          params.from_ccc === "true" || params.from_external === "true",
        title: params.customer_name
          ? `${getEventTypeLabel(
              params.intervention_type || "appointment"
    ...
    setFormData((prev) => {
      ...
      if (field === "customer_name" || field === "event_type") {
        if (
          !prev.title ||
          prev.title ===
            `${getEventTypeLabel(prev.event_type)} - ${prev.customer_name}`
        ) {
          newData.title = `${getEventTypeLabel(newData.event_type)} - ${
              ...
              <CardBody className="p-4">
                <div className="flex items-center gap-2 text-primary-800">
                  ...
                  <span className="font-medium">
                    Richiesta da Centro Contatti Cliente
                  </span>
                </div>
              </CardBody>
            </Card>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {eventTypeOptions.map((type) => (
                      <div
                        ...
                        onClick={() =>
                          handleInputChange("event_type", type.key)
                        }
                      >
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {priorityOptions.map((priority) => (
                      <div
                        ...
                        onClick={() =>
                          handleInputChange("priority", priority.key)
                        }
                      >
                        <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Select
                  ...
                  onSelectionChange={(keys) =>
                    handleInputChange("customer_type", Array.from(keys)[0])
                  }
                >
            ...
            <CardBody className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input
                  ...
                  min="15"
                  step="15"
                  description={`Durata suggerita per ${getEventTypeLabel(
                    formData.event_type
                  )}: ${
                    eventTypeOptions.find((t) => t.key === formData.event_type)
                      ?.duration || 60
                  } minuti`}
                />
                <Input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("assigned_technician", e.target.value)
                  }
                />
                <Select
                  label="Metodo di Contatto"
                  selectedKeys={
                    formData.contact_method ? [formData.contact_method] : []
                  }
                  onSelectionChange={(keys) =>
                    handleInputChange("contact_method", Array.from(keys)[0])
                  }
                >
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">
                  Invia Reminder al Cliente
                </span>
                <Switch
                  isSelected={formData.send_reminder}
                  onValueChange={(value) =>
                    handleInputChange("send_reminder", value)
                  }
                />
              </div>
            </CardBody>
          ...
          <Card className="bg-gradient-to-r from-primary-50 to-primary-100 border border-primary-200">
            <CardBody className="p-6">
              ...
              <p className="text-sm text-primary-700 mb-4">
                Dopo aver completato le informazioni, verrai portato al
                calendario dove potrai vedere tutti gli appuntamenti esistenti e
                scegliere il giorno e l'ora migliori per questo{" "}
                {getEventTypeLabel(formData.event_type).toLowerCase()}.
              </p>
